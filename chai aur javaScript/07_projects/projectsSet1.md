...
const startChangColor = () => {
  if (!intervelId) {
    intervelId = setInterval(changColor, 1000);
  }
};
const stopChangColor = () => {
  clearInterval(intervelId);
  intervelId = null;
};

start.addEventListener('click', startChangColor);
stop.addEventListener('click', stopChangColor);
